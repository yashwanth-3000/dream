...
class ExaMCPRetriever:
    ...
    async def retrieve(self, *, query: str) -> RetrievalResult:
        if not self._settings.exa_mcp_enabled:
            return RetrievalResult(
                ...
                diagnostics=RetrievalDiagnostics(
                    provider="exa_mcp",
                    errors=["EXA_MCP_ENABLED is false."],
                ),
            )
        ...
        if not mcp_url:
            return RetrievalResult(
                ...
                diagnostics=RetrievalDiagnostics(
                    provider="exa_mcp",
                    errors=["EXA_MCP_BASE_URL is not configured."],
                ),
            )

        exa_api_key = (self._settings.exa_api_key or "").strip()
        if not exa_api_key:
            return RetrievalResult(
                provider="exa_mcp",
                citations=[],
                ...
                diagnostics=RetrievalDiagnostics(
                    provider="exa_mcp",
                    errors=["EXA_API_KEY is missing."],
                ),
            )

        timeout_seconds = self._settings.exa_mcp_timeout_seconds
        headers = {
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout_seconds, headers=headers) as http_client:
                ...
                raw_packet = await self._call_search_tool(
                    http_client=http_client,
                    mcp_url=mcp_url,
            ...
            return RetrievalResult(
                ...
                diagnostics=RetrievalDiagnostics(
                    provider="exa_mcp",
                    errors=[f"Exa MCP retrieval failed: {exc}"],
                ),
            )
    ...
    async def _call_search_tool(
        ...
        *,
        ...
        attempts: list[dict[str, Any]] = [
            {"query": query, "numResults": self._settings.exa_search_top_k},
            {"query": query},
            {"search_query": query},
            {"q": query},
            {"input": query},
    ...
    def _to_citations(self, payload: Any, *, source: str) -> list[Citation]:
        ...
        for record in records:
            title = self._pick_first(record, ["title", "name"])
            url = self._pick_first(record, ["url", "uri", "source"])
            snippet = self._pick_first(record, ["snippet", "text", "content", "summary", "answer"])
            published_date = self._pick_first(record, ["published_date", "publishedDate", "date"])
            score = self._pick_first_number(record, ["score", "searchScore", "relevance"])
            ...
            if len(citations) >= self._settings.exa_search_top_k:
                break
    ...
    def _looks_like_result_record(self, record: dict[str, Any]) -> bool:
        ...
        has_text = bool({"snippet", "text", "content", "summary", "answer"} & keys)
        has_source = bool({"url", "uri", "source", "title", "name"} & keys)
    ...
    def _pick_first_number(self, record: dict[str, Any], candidate_keys: list[str]) -> float | None:
        ...
        for key in candidate_keys:
            for probe in (key, key.lower()):
                origin = probe if probe in record else lowered.get(probe.lower())
                if origin is None:
                    continue
                value = record.get(origin)
