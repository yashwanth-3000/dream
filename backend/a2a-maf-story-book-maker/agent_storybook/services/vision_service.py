...
class OpenAIVisionService:
    ...
    def describe_character_drawing(self, image_input: str, user_hint: str = "") -> str:
        instruction = (
            "Analyze this character drawing for storybook continuity. Provide concise but specific visual details "
            "covering facial traits, hair, silhouette, body proportions, outfit motifs, accessories, expression, "
            "pose, and style cues that must stay consistent across pages."
        )
        if user_hint.strip():
    ...
    def describe_world_reference(self, image_input: str, user_hint: str = "") -> str:
        instruction = (
            "Analyze this world reference image and extract storybook-relevant cues for setting consistency. "
            "Include architecture, era signals, mood, palette, texture cues, symbolic motifs, and environmental details."
        )
        if user_hint.strip():
            instruction += f" User hint: {user_hint.strip()}"
