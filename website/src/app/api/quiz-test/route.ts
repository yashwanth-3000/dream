...
function a2aOnlyTargetError(target: string) {
  return NextResponse.json(
    {
      detail: "A2A-only mode: Quiz test endpoint only supports target=main.",
      target,
...
export async function GET(request: Request) {
  ...
  const healthPath = "/api/v1/orchestrate/quiz-health";
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: "Quiz backend health check failed.",
...
export async function POST(request: Request) {
  ...
  if (stream) {
    try {
      const response = await fetch(`${baseUrl}/api/v1/orchestrate/quiz/stream${jobQuery}`, {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      return NextResponse.json(
        {
          detail: "Quiz stream request failed.",
          error: error instanceof Error ? error.message : String(error),
          backendBaseUrl: baseUrl,
          target,
        },
  ...
  try {
    const response = await fetch(`${baseUrl}/api/v1/orchestrate/quiz${jobQuery}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(bodyPayload),
      cache: "no-store",
    ...
    return NextResponse.json(
      {
        detail: "Quiz backend returned non-JSON payload.",
        raw: text,
      },
      { status: response.status }
    );
    ...
    return NextResponse.json(
      {
        detail: "Quiz backend request failed.",
        error: error instanceof Error ? error.message : String(error),
        backendBaseUrl: baseUrl,
        target,
      },
