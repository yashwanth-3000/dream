...
export default function DreamNavbar({ collapsed = false, collapsedAlign = "center" }: DreamNavbarProps) {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-3 z-50 px-4">
      <nav
        id="dream-nav"
        className="pointer-events-auto mx-auto flex w-fit items-center gap-1 rounded-full border border-white/18 px-3 py-1.5 shadow-[0_12px_28px_-16px_rgba(22,18,14,0.65)] backdrop-blur-md"
        style={{ backgroundColor: "rgb(103 99 97 / 0.92)" }}
      >
        {/* Logo */}
        <Link
          href="/"
          className="mr-2 text-[15px] text-[#f6efe8] transition-opacity duration-200 hover:opacity-80"
          style={{ fontFamily: "var(--font-halant)", fontWeight: 400, letterSpacing: "-0.04em" }}
        >
          Dream
        </Link>
        ...
        {NAV_LINKS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] transition-all duration-200 ${
                active
                  ? "bg-white/15 text-[#f6efe8]"
